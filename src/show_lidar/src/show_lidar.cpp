...
void LidarParsing()
{
    ...
    for (int j = 0; j < (int)lidar_angle.size(); j++)
    {
        ...
        {
            Point temp;
            temp.x = -1*lidar_dist[i][j] * cosTheta[i] * sinAngle[j] / 100.f;
            temp.y = -1*lidar_dist[i][j] * cosTheta[i] * cosAngle[j] / 100.f;
            temp.z = lidar_dist[i][j] * sinTheta[i] / 100.f;
            temp.r = 255;
            temp.g = 255;
            temp.b = 255;
    ...
    {
        PointT p;
        p.x = cur_lidar_pointcloud[i].x;
        p.y = cur_lidar_pointcloud[i].y;
        p.z = cur_lidar_pointcloud[i].z;
...
//    std::ifstream in("/home/lyh/lab/libpcap_test/bin/saved_0829-2_lidar_data.csv");
    ...
    std::ifstream in("/home/lyh/lab/data/huigu_1213/laser2019-12-13-19-59-6.csv");
    std::string line;
